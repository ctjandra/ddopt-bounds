...
public:
	...
	BinaryProblem(BPInstance* _inst, vector<BPProp*> _propagators, Options* _opts) : Problem(_inst, _opts)
	{
		...
		// Select merger and ordering according to options
		ordering = get_ordering_by_id_bp(options->order_id, instance, *options);
		if (ordering == NULL) {
			cout << "Error: invalid variable ordering" << endl;
			exit(1);
		}
		merger = get_merger_by_id_bp(options->merge_id, options->width);
		if (merger == NULL) {
