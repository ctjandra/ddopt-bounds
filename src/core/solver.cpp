...
BDD* DDSolver::construct_decision_diagram(SCIP* scip)
{
	BDD* bdd = construct_decision_diagram_at_state(scip, problem->create_initial_state(), 0);
	if (bdd != NULL) {
...
BDD* DDSolver::construct_decision_diagram_at_state(SCIP* scip, State* initial_state, double initial_longest_path)
{
	// Initialization
	...
	for (int layer = 0; layer < nlayers - 1; ++layer) {
		...
		// If SCIP is stopped, return no BDD
...
void DDSolver::set_dual_bound(double bound)
{
	use_dual_pruning = true;
	dual_bound = bound;
}
...
bool DDSolver::node_can_be_pruned_by_dual_bound(Problem* prob, Node* node, Node* parent)
{
	/* prune if partial solution value + completion primal bound >= dual bound */
	/* Note that we are always maximizing in a DD */
	assert(prob->completion != NULL);
	double completion_bound = prob->completion->primal_bound(prob->inst, node, parent);
	bool pruned = DBL_GE(node->shortest_path + completion_bound, dual_bound);
	// if (pruned) {
	// 	cout << "Pruned by dual bound: bound " << node->shortest_path << " + " <<  completion_bound << " = "
	// 			 << node->shortest_path + completion_bound << " >= " << dual_bound << endl;
	// }
	return pruned;
}
