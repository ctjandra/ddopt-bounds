...
# Run experiments on independent set instances of size 300.
# Experiments are run sequentially; see run_indepset_n300_parallel.sh for trivial parallelization.
...
if [ ! -d "instances/indepset_n300" ]; then
...
if [ -z "$1" ]; then
    instances="instances/indepset_n300/*.clq"
else
    instances="instances/indepset_n300/*_${1}.clq"
fi
