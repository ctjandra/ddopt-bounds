...
def shifted_geo_mean(vals, shift):
	"""Takes shifted geometric mean of values in vals with given shift"""
	if len(vals) == 0:
		print "Warning: Attempting to average empty list of values"
	l = list(vals)
	for i in range(len(l)):
		l[i] = max(l[i] + shift, 1)
	a = numpy.log(l)
	return numpy.exp(a.sum() / len(a)) - shift


# Main function that should be called externally
def collect_data(
		fields,
		instance_index,  # if None, all instances are treated individually
		path,
		userhost="",
		additional_fields={},
		filename_filter=None, 
		filename_filter_per_field=None,
		open_ssh_session=None,
		sgm_fields={},  # Fields where shifted geometric mean will be applied, with name as key and shift as value
		use_seed=True
		):
	...
	for field, t in fields.iteritems():
		...
		for line in out:
			...
			if use_seed: # Cut off seed if used
				basename, seed = basename.rsplit('-', 1)
			if instance_index:
	...
	for key, d1 in data.iteritems():
		for field, d2 in d1.iteritems():
			if field not in sgm_fields.keys():
				avg_function = numpy.mean
			else:
				shift = sgm_fields[field]
				avg_function = functools.partial(shifted_geo_mean, shift=shift)
			...
			if isinstance(d2.values()[0], list):
				...
				avg_data[key][field] = (map(avg_function, zip(*d2.values())), map(numpy.std, zip(*d2.values())))
			else:
				avg_data[key][field] = (avg_function(d2.values()), numpy.std(d2.values()))
